...
public class TicketUpdate {
	...
	public String getSysId(String user_Id) {
		...
		try {
			...
			// String encode =
			// Base64.encodeBase64(auth.getBytes("UTF-8")).toString();
	...
	public List<TicketsData> getTicketsBysystemId(String sysId, String userId, String password) {
		...
		try {
			DefaultHttpClient httpClient = new DefaultHttpClient();
			String url1 = "https://kennametal.service-now.com/api/now/table/incident?sysparm_query=assigned_to=" + sysId
			...
			try {
				...
				TicketListFromServiceNow = car.getResult();
				
				
				//Java 6 iterator for filtering the objects -- vinod raj c m
				
				/*// Create iterator to loop through each book in list.
				for (Iterator<Tickets> bookIterator = TicketListFromServiceNow.iterator(); bookIterator.hasNext();) {
					// Get next book.
				...
				TicketListFromServiceNow = TicketListFromServiceNow.stream().
										   ...
										   filter(f -> !f.getIncident_state().equals("25"))
				...
				for (Tickets tickets : TicketListFromServiceNow) {
					...
					ticketsDataTable.add(ticket_dataTable);
				}

				//new object to be added for capturing the holiday date for employee 
	...
	public @ResponseBody String runCronJob4(HttpServletRequest request, HttpServletResponse response)
		...
		/*
		 * byte[] asB64; //asB64 = Base64.getDecoder().decodeToString(
		 * "VG9wIERyaWxsIE0x4oSiIE1vZHVsYXIgRHJpbGwgSW5zZXJ0cyDigKIgTGVzcyBUaGFuIC41NTEiICgxNCBtbSk="
		 * .getBytes("utf-8")); asB64 = Base64.getDecoder().decode(
		 * "REZSIFdoaXN0bGUgTm90Y2ggKFdEKSBTaGFuayDigKIgMyB4IEQ=".getBytes(
		 * "utf-8"));
		 * 
		 * 
		 * System.out.println(new String(asB64, StandardCharsets.UTF_8));
		 * 
		 * return "";
		 */
